...
- (AVCaptureSession *)session {
    if (!_session) {
        ...
        CGRect rect = CGRectMake(kBgImgY/ScreenHeightes, kBgImgX/ScreenWidthes, kBgImgWidth/CGRectGetHeight([UIScreen mainScreen].bounds), kBgImgWidth/ScreenWidthes);
    ...
    if (metadataObjects.count > 0) {
        ...
        if (obj) {
           ...
           // if (_showQRCodeInfo) {
                ...
                [self accordingQcode:[obj stringValue]];
           // };
...
- (void)accordingQcode:(NSString *)str
{
    [self.session stopRunning];
    //在这里进行跳转界面并进行回调
    if ([self.qcodeString isEqualToString:@"0"]) {
       
    }
    else if([self.qcodeString isEqualToString:@"1"]){
        
       
    }else if ([self.qcodeString isEqualToString:@"2"]){//添加商品
        if (self.block) {
            self.block(str);
